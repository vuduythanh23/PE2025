...
export const navigateToProducts = (navigate, additionalState = {}) => {
  ...
  if (savedState) {
    ...
    if (savedState.priceRange?.min)
      params.set("minPrice", savedState.priceRange.min);
    if (savedState.priceRange?.max)
      params.set("maxPrice", savedState.priceRange.max);
    if (savedState.sortBy) params.set("sort", savedState.sortBy);
    if (savedState.currentPage && savedState.currentPage > 1)
      params.set("page", savedState.currentPage);
