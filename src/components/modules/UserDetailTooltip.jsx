        ...
        <ClientOnly>
          <div className="absolute z-50 w-80 p-4 bg-white border border-gray-200 rounded-lg shadow-lg top-full left-0 mt-2">
            <div className="flex items-start space-x-3">
              <div className={`flex-shrink-0 h-12 w-12 rounded-full flex items-center justify-center text-sm font-medium text-white ${
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="space-y-1 text-xs text-gray-600">
                  ...
                  <div><span className="font-medium">ID:</span> {user._id}</div>
                </div>
                ...
                {(user.failedLoginAttempts > 0 || user.lastFailedAttemptAt) && (
                  <div className="mt-2 p-2 bg-orange-50 rounded text-xs">
                    <div className="text-orange-800 font-medium">Security Info:</div>
                    {user.failedLoginAttempts > 0 && (
                      <div>Failed login attempts: {user.failedLoginAttempts}</div>
                    )}
                    {user.lastFailedAttemptAt && (
                      <div>Last failed attempt: {new Date(user.lastFailedAttemptAt).toLocaleString()}</div>
                    )}
                  </div>
                )}
              </div>
