  ...
  return (
    <div className="space-y-8">
      ...
      {/* Brand Filter */}      <div>
        ...
        <select
          value={tempFilters.brand}          onChange={(e) => {
            ...
            if (brandId) {
              ...
              const selectedBrand = brands.find(b => {
                ...
                if (match) {
                  console.log(`Brand match found: ${b.name} (${bId}) === ${brandId}`);
                }
                return match;
              });
              ...
              if (!selectedBrand) {
                console.warn(`Brand with ID ${brandId} not found in brands list!`);
              }
            }
            
            // Show summary of all brands for debugging
            console.log(`All brands (${brands.length}):`, brands.map(b => ({
        ...
        <h3 className="text-lg font-serif text-luxury-dark mb-4">
          ...
          value={
            tempFilters.priceRange.min === null
              ? "all"
              : `${tempFilters.priceRange.min}-${tempFilters.priceRange.max === Infinity ? 'up' : tempFilters.priceRange.max}`
          }          onChange={(e) => {
            const selectedValue = e.target.value;
            console.log("Selected price range value:", selectedValue);
            const selected = priceRanges.find(range => range.id === selectedValue);
            console.log("Selected price range object:", selected);
            
            if (selected) {
              const newPriceRange = selected.value === "all" 
                ? { min: null, max: null } 
