...
export default function CategoryQuickFilter() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      console.log("Available categories:", categoriesArray.map(cat => ({
        id: cat._id || cat.id,
        name: cat.name,
        type: cat.type,
        parent: cat.parent
  ...
  const handleCategoryClick = (categoryKey) => {
    ...
    const mainCategory = categories.find(cat => {
      ...
      if (searchKey === 'men' && (categoryName.includes('men') || categoryName.includes('male'))) return true;
      if (searchKey === 'women' && (categoryName.includes('women') || categoryName.includes('female') || categoryName.includes('woman'))) return true;
      if (searchKey === 'kids' && (categoryName.includes('kids') || categoryName.includes('children') || categoryName.includes('child'))) return true;
      
      return false;
    });

    if (mainCategory) {
      console.log(`Navigating to category: ${mainCategory.name} (${mainCategory._id})`);
      navigate(`/products?category=${encodeURIComponent(mainCategory._id || mainCategory.id)}`);
    } else {
      ...
      if (fallbackCategory) {
        console.log(`Fallback navigation to: ${fallbackCategory.name} (${fallbackCategory._id})`);
        navigate(`/products?category=${encodeURIComponent(fallbackCategory._id || fallbackCategory.id)}`);
      } else {
    ...
    <div className="w-full max-w-6xl mx-auto">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 md:gap-10">
        {mainCategories.map((category) => (
          <button
            key={category.key}
            onClick={() => handleCategoryClick(category.key)}            className={`
